...
zstd-devel\
"
export PACKAGES="qgis-dev qgis-dev-deps qgis-dev-full qgis-dev-full-free qgis-dev-full-grids qgis-dev-pdb"
...
if [ -d qgis ]; then
  ...
  cd src/plugins
  git clone git@github.com:benoitdm-oslandia/plugin-builder.git primitive_editing
  cd ../..
elif [ -n "$REF" ]; then
  set -x
  mkdir qgis
...
(
  ...
  export GRASS_VERSION=$(
    unset SHELL
    cmd /c $GRASS --config version | sed -e "s/\r//"
  )
  export GRASS_PREFIX=$(
    unset SHELL
    cmd /c $GRASS --config path | sed -e "s/\r//"
  )
  ...
  cmake -G Ninja \
    ...
    -D WITH_SERVER=FALSE \
    -D WITH_SFCGAL=FALSE \
    -D CUSTOM_PLUGINS=primitive_editing \
    $(cygpath -m $SRCDIR)
