...
(
	...
	packages='"" "-common" "-server" "-devel" "-oracle-provider" "-grass-plugin-common"'
	...
	mkdir -p                                                                                 install/apps/$P/pyhton/PyQt5/uic/widget-plugins
	...
	mkdir -p $R/$P-{pdb,full,common,server}
...
EOF
	...
	/bin/tar -C install -cjf $R/$P-pdb/$P-pdb-$V-$B.tar.bz2 \
		apps/$P/pdb
